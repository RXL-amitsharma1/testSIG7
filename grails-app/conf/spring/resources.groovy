...
import com.rxlogix.security.jwt.JwtRepository
import com.rxlogix.security.AuthorizationAspect
...
import com.rxlogix.websocket.CustomWebSocketConfig
...
beans = {
    ...
    if (Holders.config.grails?.plugin?.springsecurity?.saml?.active) {
        // Retrieve the login endpoint URL
        def loginEndpointURL = grailsApplication.config.grails.plugin.springsecurity.saml.metadata.providers.find {
            // Assuming you need the first one or specify some condition
            it.key
        }?.key
...
// Check if the URL was retrieved successfully
    ...
    jwtRepository(JwtRepository) { bean ->
        bean.constructorArgs = [
                ref('hazelcastInstance'),
                Duration.parse('PT' + grailsApplication.config.getProperty('jwt.ttl', String))
        ]
    }

    jwtService(JwtService) { bean ->
        bean.constructorArgs = [
                grailsApplication.config.getProperty('jwt.secretKey', String),
                Duration.parse('PT' + grailsApplication.config.getProperty('jwt.ttl', String)),
                grailsApplication.config.getProperty('jwt.issuer', String),
                ref('jwtRepository')
        ]
    }
    ...
    customWebsocketConfig(CustomWebSocketConfig)

    securityHeadersFilter(SecurityHeadersFilter)//{
////        filter = new com.rxlogix.security.SecurityHeadersFilter()       // Create a new instance of com.rxlogix.security.SecurityHeadersFilter
////        urlPatterns = ['/*']
