...
grails.config.locations = ["classpath:application_4.groovy",
                           "classpath:application_3.groovy",
                           "classpath:application_two.groovy" ,
                           "file:${userHome}/.signal/config.groovy"]
    ...
    development {
        ...
        dataSources {
            dataSource {
                ...
                dialect = 'org.hibernate.dialect.PostgreSQLDialect'
                url = 'jdbc:postgresql://127.0.0.1:5432/postgresl'
                username = 'pvs'
...
grails.plugin.databasemigration.updateOnStartFileName = ['changelog_release_7.0.groovy']
...
audit.custom.module.names = ["Action","Action Configuration","Action Template","Action Type","Ad-Hoc Alert Configuration",
                             ...
                             "Signal: Email","Signal Email log: Export","Signal: Export","Signal: Generated Chart","Signal: Individual Case Review Observations","Signal: Literature Review Observations","Signal: RMMs",
...
rxlogix.security.saml.provider.name = 'pvsignal'
pvsignal {
    ...
    csrfProtection {
        enabled = true
        excludeURLPatterns = ["/saml/", "/login/saml2/sso/${rxlogix.security.saml.provider.name}", "/saml2/authenticate/${rxlogix.security.saml.provider.name}",
                              "/logout/saml2/sso/${rxlogix.security.saml.provider.name}",
                              "/saml/SingleLogout", "/saml/logout", "/console/", "/j_spring_security_check", '/odata', '/spotfire/rx_validate',
                              '/tags', '/adHocAlert' , '/executedCaseSeries', '/token','/batchlot/import', '/runEtl', '/productGroup','/productGroup/saveProductGroup',
                              '/login/authenticate','/login/authAjax','/scim/','/applicationNotification/','/businessConfig/', '/fieldManagementRefresh/',
                              '/viewInstanceRest/','/aggregateCaseAlertRest/', '/commonTagRest/', '/queryRest/', '/advancedFilterRest/','/alertCommentRest/',
                              '/productEventHistoryRest/',"/validatedSignal/", "/activityRest/", "/jwtAuth/", "/inboxLogs/", "/actionRest/","/stomp/",'/aggregateCaseAlert/']
    }
...
signal.qualitative.report.job.interval = 20000l
...
signal.scaColumnList = [[name: "flags", display: "Flags", dataType: 'java.lang.String'],
                        ...
                        [name: "coPackagedProduct", display: "Co-Packaged Product", dataType: 'java.lang.String'],
]
...
signal.scaOnDemandColumnList = [[name: "productName", display: "Product Name", dataType: 'java.lang.String'],
                                ...
                                [name: "allPTsOutcome", display: "All PTs - Outcome",isAutocomplete: true , dataType: 'java.lang.String'],
                                [name: "crossReferenceInd", display: "Cross Reference IND",isAutocomplete: true , dataType: 'java.lang.String'],
                                [name: "coPackagedProduct", display: "Co-Packaged Product",isAutocomplete: true , dataType: 'java.lang.String']
]
...
grails.plugin.springsecurity.saml.metadata.providers = ["${rxlogix.security.saml.provider.name}": "${userHome}/.reports/client-tailored-saml-idp-metadata.xml"]
...
grails.plugin.springsecurity.saml.forceAuthN = false
...
grails.plugin.springsecurity.controllerAnnotations.staticRules = [
        ...
        [pattern: '/saml/SingleLogout/**', access: ['permitAll']],
        ...
        [pattern: '/auditLogEvent/**', access: ['ROLE_ADMIN']],
        ...
        [pattern: '/dashboard/fetchTopNavAndSidePanelDetails', access: ['permitAll']],
        [pattern: '/appUiConfig/**', access: ['permitAll']],
        [pattern: '/fieldManagementRefresh/**', access: ['permitAll']],
        [pattern: '/viewInstanceRest/**', access: ['permitAll']],
        [pattern: '/aggregateCaseAlertRest/**', access: ['permitAll']],
        [pattern: '/activityRest/**', access: ['permitAll']],
        [pattern: '/validatedSignal/**', access: ['permitAll']],
        [pattern: '/commonTagRest/**', access: ['permitAll']],
        [pattern: '/queryRest/', access: ['permitAll']],
        [pattern: '/advancedFilterRest/**', access: ['permitAll']],
        [pattern: '/jwtAuth/**', access: ['permitAll']],
...
// https://github.com/spring-cloud/spring-cloud-commons/issues/897
    cloud.'decrypt-environment-post-processor'.enabled = false
}

server {
    servlet.'context-path' = '/signal'
    port = 7171
}
product.view.assignment.migration.required = false
jwt.secretKey = '{cipher}bRNWMGPnEcKrvMTx4WsIOw=='
jwt.ttl = '240h'
jwt.issuer = 'pvsignal'
signal.angular.app.base.url = 'http://localhost:4200'
signal.angular.app.alert.details = '/alerts/aggregateCaseAlert/details'

literature.embase.url = 'https://api.elsevier.com'
literature.embase.path = '/content/embase/article'
literature.embase.apiKey = 'd8125f96d0cbe6d9c0834a2cd0317e6a'
literature.embase.insttoken = '59003bf059d9ec8f82b30a4bf6bb5abf'
alert.disposition.useDropdown=false
aws {
    aggregate {
        lambda {
            arn = "arn:aws:lambda:us-west-1:857914659558:function:pvs-dev-rn-safety-initiate_adhoc_alert_lmd"
            timeout = 60        // Minutes
            interval = 5        // Minutes
        }
        execution.queue.size = 50
        datalake = "pvs_dev_rn_safety_alertoutputdatalake" //"s3://pvs-siab-safety/pvs_siab_icebergdatalake"
        region = "us-west-1"
        bucket.name = "pvs-dev-rn-safety"
        execution.enabled = false
    }
}
product.assignment.alert.group = [["name":"AG1", "grouping":"false"],["name":"AG2", "grouping":"false"], ["name":"AG3", "grouping":"false"], ["name":"AG4", "grouping":"false"]]
is.secondary.assignee = true
postgresql.sensitive.data = [
        "single_case_alert" : ["date_of_birth"]
]