...
databaseChangeLog = {
    changeSet(author: "rishabh (generated)", id: "20211209141656-1") {
        ...
        addColumn(tableName: "RCONFIG") {
            column(name: "DATA_MINING_VARIABLE", type: "VARCHAR(255 )") {
    ...
    changeSet(author: "rishabh (generated)", id: "1608626578696-1") {
        ...
        addColumn(tableName: "PREFERENCE") {
            column(name: "API_TOKEN", type: "VARCHAR(4000 )") {
                constraints(nullable: "true")
            }
        }
    }
    ...
    changeSet(author: "rishabh (generated)", id: "1608626578695-103") {
        preConditions(onFail: 'MARK_RAN') {
            not {
                sqlCheck(expectedResult: '0', "SELECT COUNT(1) FROM AGG_ALERT WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE)")
            }
        }
        sql("""
        UPDATE AGG_ALERT
        SET DUE_DATE = NULL
        WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE);

        UPDATE ARCHIVED_AGG_ALERT
        SET DUE_DATE = NULL
        WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE);
    """)
    }
    ...
    changeSet(author: "rishabh (generated)", id: "1608626578695-104") {
        preConditions(onFail: 'MARK_RAN') {
            not {
                sqlCheck(expectedResult: '0', "SELECT COUNT(1) FROM SINGLE_CASE_ALERT WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE)")
            }
        }
        sql("""
        UPDATE SINGLE_CASE_ALERT
        SET DUE_DATE = NULL
        WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE);

        UPDATE ARCHIVED_SINGLE_CASE_ALERT
        SET DUE_DATE = NULL
        WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE);
    """)
    }
    ...
    changeSet(author: "rishabh (generated)", id: "1608626578695-105") {
        preConditions(onFail: 'MARK_RAN') {
            not {
                sqlCheck(expectedResult: '0', "SELECT COUNT(1) FROM EVDAS_ALERT WHERE DISPOSITION_ID IN (SELECT ID FROM DISPOSITION WHERE CLOSED=TRUE OR REVIEW_COMPLETED=TRUE)")
            }
        }
        sql("""
        UPDATE EVDAS_ALERT
    ...
    changeSet(author: "Kundan.Kumar (generated)", id: "1632254096072-122") {
        ...
        createTable(tableName: "PROD_GROUPS_STATUS") {
            ...
            column(name: "UNIQUE_IDENTIFIER", type: "VARCHAR(300 )") {  constraints(nullable: "false")  }
    ...
    changeSet(author: "Kundan.Kumar (generated)", id: "1632254096072-124") {
        ...
        createTable(tableName: "PROD_GROUPS_DATA") {
            ...
            column(name: "GROUP_OLD_NAME", type: "VARCHAR(300 )")
            column(name: "DESCRIPTION", type: "VARCHAR(4000 )")
            column(name: "COPY_GROUPS", type: "VARCHAR(300 )")
            column(name: "SHARED_WITH", type: "VARCHAR(300 )")
            column(name: "OWNER", type: "VARCHAR(300 )")
            column(name: "INCLUDE_SOURCES", type: "VARCHAR(1000 )")
    ...
    changeSet(author: "yogesh (generated)", id: "1632254096072-128") {
        ...
        addColumn(tableName: "PROD_GROUPS_STATUS") {
            column(name: "PVR_ERROR", type: "VARCHAR(8000 )") {
                constraints(nullable: "true")
            }
        }
    }
