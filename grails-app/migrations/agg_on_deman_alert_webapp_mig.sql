...
BEGIN
    ...
    FROM
        user_tables
    WHERE
        table_name = 'CAT_AGG_ON_DEMAND_ALERT';

    IF ln_count = 1 THEN
        lvc_sql := 'DROP TABLE cat_agg_on_demand_alert PURGE';
        EXECUTE IMMEDIATE lvc_sql;
    END IF;
    lvc_sql := '
    CREATE TABLE cat_agg_on_demand_alert
        AS
            SELECT DISTINCT
    ...
    EXECUTE IMMEDIATE lvc_sql;
    lvc_sql := '
    CREATE INDEX idx_cat_agg_on_demand_alert ON
        cat_agg_on_demand_alert (
    ...
    SET
        tgt.event_hierarchy_id = (
            ...
            FROM
                cat_agg_on_demand_alert alertab
            WHERE
