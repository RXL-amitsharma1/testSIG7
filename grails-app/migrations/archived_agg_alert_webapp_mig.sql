...
BEGIN
    ...
    FROM
        user_tables
    WHERE
        table_name = 'CAT_ARCHIVED_AGG_ALERT';

    IF ln_count = 1 THEN
        lvc_sql := 'DROP TABLE cat_archived_agg_alert PURGE';
        EXECUTE IMMEDIATE lvc_sql;
    END IF;
    lvc_sql := '
    CREATE TABLE cat_archived_agg_alert
        AS
            SELECT DISTINCT
    ...
    EXECUTE IMMEDIATE lvc_sql;
    lvc_sql := '
    CREATE INDEX idx_cat_archived_agg_alert ON
        cat_archived_agg_alert (
    ...
    SET
        tgt.event_hierarchy_id = (
            ...
            FROM
                cat_archived_agg_alert alertab
            WHERE
