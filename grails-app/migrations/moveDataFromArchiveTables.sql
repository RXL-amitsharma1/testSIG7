create or replace PROCEDURE MOVE_ALERT_FROM_ARCHIVE(current_exe_id IN NUMBER, pre_exe_id IN NUMBER,
                                                    alert_config_id IN NUMBER, alert_type VARCHAR) AS
  type typename_type is table of VARCHAR(32767);
  typename typename_type;
  v_code NUMBER;
  lvc_sql VARCHAR(32000);
  lvc_exec_sql VARCHAR(32000);
BEGIN
  ...
  if (alert_type = 'Literature Search Alert') then
    BEGIN
      ...
      INSERT into LIT_ALERT_ACTIONS(LITERATURE_ALERT_ID, ACTION_ID, IS_RETAINED)
      SELECT ala.ARCHIVED_LIT_ALERT_ID, ala.ACTION_ID, ala.IS_RETAINED
  ...
  if (alert_type = 'EVDAS Alert') then
    BEGIN
      ...
      INSERT into EVDAS_ALERT_ACTIONS(EVDAS_ALERT_ID, ACTION_ID,IS_RETAINED)
      SELECT veva.ARCHIVED_EVDAS_ALERT_ID, veva.ACTION_ID, veva.IS_RETAINED
