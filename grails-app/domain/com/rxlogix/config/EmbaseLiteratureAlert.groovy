...
class EmbaseLiteratureAlert extends BaseLiteratureAlert implements AlertUtil {
    ...
    static mapping = {
        table name: 'EMBASE_LITERATURE_ALERT'
        def superMapping = BaseLiteratureAlert.mapping.clone()
        superMapping.delegate = delegate
        superMapping.call()

        validatedSignals joinTable: [name: "VALIDATED_EMBASE_LITERATURE_ALERTS", column: "VALIDATED_SIGNAL_ID", key: "EMBASE_LITERATURE_ALERT_ID"]
        alertTags joinTable: [name: "EMBASE_LITERATURE_ALERT_TAGS", column: "ALERT_TAG_ID", key: "EMBASE_LITERATURE_ALERT_ID"]
        actions joinTable: [name: "EMBASE_LITERATURE_ALERT_ACTIONS", column: "ACTION_ID", key: "EMBASE_LITERATURE_ALERT_ID"]
        pvsAlertTag joinTable: [name: "EMBASE_LITERATURE_CASE_ALERT_TAGS", column: "PVS_ALERT_TAG_ID", key: "EMBASE_LITERATURE_ALERT_ID"]
    }
    ...
    def isDefaultState() {
        if (!isDispChanged && getDispositionById(this.disposition.id) == getDefaultDisp() && this.dispPerformedBy == null) {
            return 'true'
        }
        return 'false'
    }
