            ...
            <ul id="detail-tabs" class="validation-tab m-b-5 p-0">
                ...
                <li role="presentation">
                    <a href="#details" id="detailsTab" aria-controls="details" role="tab" data-toggle="tab"
                       accesskey="2">
                        <g:message code="app.label.signal.management.review" default="Safety Observations"/>
                    </a>
                </li>
    ...
    <script>
        ...
        $(function () {
         ...
         $("#editActionModal").find("#config").select2();
            $('#detail-tabs a').on('click', function (e) {
                e.preventDefault();
            });
            $(".dueDateClass").hide();
            $(document).on('click', '#assessmentTab', function () {
                var selectedDatasource=$('#selectDatasource').val();
                if(selectedDatasource!= null && selectedDatasource != undefined && selectedDatasource !== 'pva'){
                $('.data-analysis-assessment').addClass('disabled');
                $(".data-analysis-assessment").css({
                        "pointer-events": "none",
                        "cursor": "not-allowed"
                    });
                }
            ...
            $(document).on('click', '#detail-tabs a', function (event) {
                ...
                if (activeTab == '#notifications') {
                    setTimeout(function () {
                        _.each($('#notifications .dt-scroll-head'), function (elem) {
        ...
        $('#accordion-pvs-analysis-toggle').on('click', function() {
            $('#signalActivityTable').DataTable().ajax.reload();
        });
        $(".editButtonEvent").on('click',
            function (e) {
                eventObj = e;
