...
<script>
    $(function(){
        ...
        function disableLink(link,event){
        ...
        <sec:ifAnyGranted roles="ROLE_ADMIN">
            <div class="navScaffold col-xs-12">
                <g:if test="${entityName == 'Group'}">
                    <g:link id="createGroup" name="createGroup" class="createGroup btn btn-primary" action="create" params="[ type: 'workflow' ]">
                        <span class="glyphicon glyphicon-plus icon-white"></span>
                        <g:message code="default.new.label.workflow.group"/>
                    </g:link>
                    <g:link id="createUser" class="createUser btn btn-primary" action="create" params="[ type: 'user' ]">
                        <span class="glyphicon glyphicon-plus icon-white"></span>
            ...
            <g:if test="${groupInstanceTotal > 0}">
                ...
                <g:each in="${groupInstanceList}" status="i" var="groupInstance">
                    <tr>
                        ...
                        <td class="col-min-150 col-max-150">${groupInstance.groupType.value}</td>
                        <td class="dataTableColumnCenter col-min-150 col-max-150 forceLineWrapDateAlign">${com.rxlogix.util.DateUtil.toDateStringWithTimeInAmPmFormat(groupInstance.lastUpdated,  userService.getUser()?.preference?.timeZone)}</td>
                        <td class="dataTableColumnCenter col-min-150 col-max-150 forceLineWrapDateAlign">${com.rxlogix.util.DateUtil.toDateStringWithTimeInAmPmFormat(groupInstance.dateCreated, userService.getUser()?.preference?.timeZone)}</td>
                        <td>${groupInstance.createdBy}</td>


                        <td class="col-min-100 col-max-100 no-padding"><div class="hidden-btn btn-group dataTableHideCellContent" align="center">
