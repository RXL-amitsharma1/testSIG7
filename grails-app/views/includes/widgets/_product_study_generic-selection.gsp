...
<g:each in="${alertCriteriaList}" var="entry">
<g:if test="${entry?.enabled == true && entry?.name == 'productSelection'}">
    <label class="radio-inline labelBold m-b-5" id = "productSelectionLabel">
    ...
    <label class="labelBold m-b-5" id = "productHierarchyMasterTemplateLabel" hidden="hidden">
        <g:message code="app.label.product.hierarchy.label"/>
    </label>
    ...
    <g:if test="${!(theInstance instanceof com.rxlogix.config.LiteratureConfiguration)}">
        ...
        %{--</label>--}%
        <g:if test="${entry?.enabled == true && entry?.name == 'studySelection'}">
        ...
        <g:if test="${grails.util.Holders.config.study.dictionary.enabled}">
            <g:if test="${entry?.enabled == true && entry?.name == 'study'}">
        ...
        <g:set var="notLiterature" value="true" />
    </g:if>
</g:each>
    <%
        // Groovy logic to check if any of the required fields are mandatory
        boolean isMandatory = alertCriteriaList.any { entry ->
            (entry?.name == 'productSelection' || entry?.name == 'studySelection' || entry?.name == 'study') && entry?.mandatory
        }
    %>

<!-- Display the * indicator at the end if any field is mandatory -->
    <g:if test="${isMandatory && notLiterature}">
...
<g:else>
    <div class="${hasErrors(bean: theInstance, field: 'productSelection', 'has-error')}">
        ...
        <g:else>
            <label class="labelBold m-b-5" id = "productSelectionLabel">
                <g:message code="app.label.product.label"/>
            </label>
        </g:else>
    </div>
...
<div class="col-md-12">
    <div class="wrapper" id="productSelectionDictionarySection">
...
%{-- Changes done as per PVS-54685 bug --}%
<g:if test="${!(theInstance instanceof com.rxlogix.config.LiteratureConfiguration)}">
...
<g:javascript>
    $(function () {
        ...
        $("#limitToSuspectProduct").on('click', function () {
