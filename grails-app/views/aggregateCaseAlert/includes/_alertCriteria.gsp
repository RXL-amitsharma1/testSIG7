<%@ page import="grails.util.Holders; com.rxlogix.Constants; com.rxlogix.util.DateUtil; grails.plugin.springsecurity.SpringSecurityUtils; com.rxlogix.util.ViewHelper; org.springframework.security.core.context.SecurityContextHolder" %>
<g:set var="productAssignmentService" bean="productAssignmentService"/>
<div class="row" id = "aggConfigurationAlertCriteria">

    <div class="col-md-12">
        <div class="row">
            <g:render template="/includes/widgets/product_study_generic-selection" bean="${configurationInstance}"
                      model="[theInstance: configurationInstance, showHideGenericVal: 'hide', showHideStudyVal: 'hide', productHierarchy: productHierarchy, eventHierarchy: eventHierarchy, configurationInstanceEventHierarchy: configurationInstanceEventHierarchy, configurationInstancePrdHierarchy: configurationInstancePrdHierarchy]"/>


            <div class="col-md-3">
                <label id="eventDictionarySelectionLabel"><g:message code="app.label.eventSelection"/><span
                        class="required-indicator"/><span class="required-indicator"/></label>
                ...
                <div class="wrapper" id="eventDictionarySelection">
                    <div id="showEventSelection" class="showDictionarySelection"></div>

                    <div class="iconSearch">
                        <a id="searchEvents" data-toggle="modal" data-target="#eventModal" tabindex="0" role="button"
                           data-toggle="tooltip" title="Search Event" accesskey="}"><i class="fa fa-search"></i></a>
                    </div>
                </div>
                        ...
                        <div class="fuelux">
                            <div class="datepicker toolbarInline" id="asOfVersionDatePicker" hidden="hidden">
                                <div class="input-group">
                                    <g:hiddenField name="asOfVersionDateValue"
                                                   value="${configurationInstance?.asOfVersionDate ?: null}"/>
            ...
            <div class="col-md-3">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <table class="table table-striped pv-list-table dataTable no-footer">
                                    ...
                                    <tr>
                                        <td><g:message code="reportCriteria.include.study.cases"/></td>
                                        <td><g:message code="reportCriteria.include.study.cases.description"/></td>
                                    </tr>
                                    <tr>
                                        <td><g:message code="reportCriteria.ignore.study.medication.type"/></td>
                                        <td><g:message
                                                code="reportCriteria.ignore.study.medication.type.description"/></td>
                                    </tr>
                                    <tr>
                                        <td><g:message code="reportCriteria.consider.only.poi"/></td>
                                        <td><g:message code="reportCriteria.consider.only.poi.description"/></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                <label><g:message code="app.label.inclusionOptions"/></label>  <a href="javascript:void(0)"
                                                                                  class="glyphicon glyphicon-info-sign themecolor"
                                                                                  data-toggle="modal"
                                                                                  data-target="#myModal"></a>
                <g:if test="${!signalId && SecurityContextHolder.context.authentication.authorities*.authority.contains('ROLE_DATA_MINING_ADVANCED_RUNS')}">
                    <div style="display: flex; align-items: center;">
                        <div class="checkbox checkbox-primary">
                            <g:checkBox id="isMasterTemplateAlert"
                                        name="isMasterTemplateAlert"
                                        value="${configurationInstance?.isMasterTemplateAlert}"
                                        checked="${configurationInstance?.isMasterTemplateAlert}"
                                        disabled = "${!configurationInstance?.isMasterTemplateAlert}"/>

                            <label for="isMasterTemplateAlert">
                                <g:message code="app.label.MastertemplateAlert"/>
                            </label>
                        </div>

                        <div class="wrapper" id="selectedProductHierarchy" hidden="hidden"
                             style="width: 150px; margin-left: 5px;">
                            <select id="productHierarchySelectDropDown" name="productHierarchy"
                                    class="form-control productHierarchy">
                                <option value="">None</option>
                                <g:each in="${productHierarchy}" var="hierarchy">
                                    <option value="${hierarchy}" ${hierarchy == configurationInstancePrdHierarchy ? 'selected' : ''}>${hierarchy}</option>
                                </g:each>
                            </select>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; align-items: center;">
                        <div class="checkbox checkbox-primary">
                            <g:checkBox id="alertGroupCheckBox"
                                        name="alertGroupCheckBox"
                                        value="${configurationInstance?.isMasterTemplateAlert}"
                                        checked="${configurationInstance?.excludedAlertGroups ?true:false}"
                                        disabled = "${!configurationInstance?.isMasterTemplateAlert}"/>
                            <label for="alertGroupCheckBox">
                                <g:message code="exclude.alert.groups"/>
                            </label>
                        </div>

                        <div class="wrapper" id="alertGroupDiv" hidden="hidden"
                             style="width: 200px; margin-left: 5px">
                            <g:select id="alertGroup" name="alertGroup" multiple="multiple"
...
%{--                </div>--}%
                <div style="display: flex">
%{--                    Fake Check Box Added For Alignment as suggested by Product Team.--}%
                    <div class="checkbox checkbox-primary">
                        <g:checkBox id="groupBySmqCheckBox"
                                    name="groupBySmqCheckBox"
                                    checked="true"
                                    disabled="true"/>
                    <label for="groupBySmqCheckBox"></label>
                    </div>
                    <label for="groupBySmq" id="groupBySmqLabel">
                        <g:message code="reportCriteria.groupBySmq"/>
                    </label>
                    <select id="groupBySmq" name="groupBySmq" class="form-control" style="width: 150px; margin-left: 5px;">
                        <option value="PT" ${configurationInstance?.groupBySmq ? '' : 'selected'}>PT</option>
                        <option value="SMQ/EventGroup" ${configurationInstance?.groupBySmq ? 'selected' : ''}>SMQ/EventGroup</option>
                    </select>
                </div>
                ...
                <div class="checkbox checkbox-primary">
                    <g:checkBox id="studyDrugCases"
                                name="studyDrugCases"
                                value="${configurationInstance?.studyCases}"
                                checked="${configurationInstance?.studyCases}"
                                disabled="${!action.equals('edit') && !action.equals('copy')}"/>
                    <label for="studyDrugCases">
                        <g:message code="app.label.configuration.study.cases"/>
                    </label>

                </div>

                <div id="additionalOptions"
                     class="${action.equals("create") || !configurationInstance?.studyCases ? 'hidden' : ''}"
                     style="margin-left: 20px">
                    <div class="checkbox checkbox-primary">
                        <g:checkBox id="ignoreMedicationType"
                                    name="ignoreMedicationType"
                                    value="${configurationInstance?.ignoreStudyType}"
                                    checked="${configurationInstance?.ignoreStudyType}"/>
                        <label for="ignoreMedicationType">
                            <g:message code="app.label.configuration.medication.ignore"/>
                        </label>

                    </div>

                    <div class="checkbox checkbox-primary">
                        <g:checkBox id="considerPOI"
                                    name="considerPOI"
                                    value="${configurationInstance?.onlyPoi}"
                                    checked="${configurationInstance?.onlyPoi}"/>
                        <label for="considerPOI">
                            <g:message code="app.label.configuration.poi"/>
                        </label>
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <label><g:message code="app.label.choseQuery.aggregate"/></label> <a href="javascript:void(0)"
                                                                                     class="glyphicon glyphicon-info-sign themecolor open-background-query"
                                                                                     data-toggle="tooltip"
                                                                                     title="Background Query help"></a>
                <!-- Modal -->
                ...
                <div class="row queryContainer m-l-0">
                    ...
                    <g:hiddenField name="alertQueryName" value="${configurationInstance?.alertQueryName}"
                                   id="alertQueryName"/>
                    <g:hiddenField name="alertQueryId" value="${configurationInstance?.alertQueryId}"
                                   id="alertQueryId"/>
                </div>
            </div>

            <div class="col-md-1 bgQueryIcon">
                <a target="_blank" class="glyphicon pull-left glyphicon-info-sign viewBgQuery" data-toggle="tooltip"
        ...
        <div class="row">
            <div class="alertQueryWrapper col-md-9 p-lr-0">
                            ...
                            <g:each var="qev" in="${qvl.parameterValues}" status="j">
                                <g:if test="${qev?.hasProperty('reportField')}">
                                    <g:render template='/query/toAddContainerQEV'
                                              model="['qev': qev, 'type': 'qev', 'i': i, 'j': j]"/>
                                </g:if>
                                <g:else>
                ...
                <g:hiddenField class="validQueries" name="validQueries"
                               value="${configurationInstance ? configurationInstance.getQueriesIdsAsString() : ''}"/>
            </div>
        </div>
        <input type="hidden" name="foregroundSearch" id="foregroundSearch"
               value="${configurationInstance.foregroundSearch}" class="m-l-5">
        <input type="hidden" name="foregroundSearchAttr" id="foregroundSearchAttr"
               value="${configurationInstance.foregroundSearchAttr}">
        <input type="hidden" id="alertType" value="${configurationInstance.type}">
        <div class="row foregroundQueryCheckbox" style="margin-left: 10px; margin-top: 20px;">
...
<input type="hidden" id="isIncludeLockedVersions"
       value="${Holders.config.configurations.include.locked.versions.enabled}"/>
...
<g:if test="${actionName == 'edit'}">
    ...
    <div>
        <label class="text-grey"><i>${Constants.Commons.LAST_EXECUTED_DATE_RANGE}: ${lastExecutionDate}</i></label>
    </div>
    <g:if test="${exeEvdasDateRange}">
        <div>
            <label class="text-grey"><i>${Constants.Commons.LAST_EXECUTED_DATE_RANGE} (EVDAS): ${exeEvdasDateRange}</i>
            </label>
        </div>
    </g:if>
    <g:if test="${exeFaersDateRange}">
        <div>
            <label class="text-grey"><i>${Constants.Commons.LAST_EXECUTED_DATE_RANGE} (FAERS): ${exeFaersDateRange}</i>
            </label>
        </div>
    </g:if>
    <g:if test="${exeVigibaseDateRange}">
        <div>
            <label class="text-grey"><i>${Constants.Commons.LAST_EXECUTED_DATE_RANGE} (VigiBase): ${exeVigibaseDateRange}</i>
            </label>
        </div>
    </g:if>
    <g:if test="${exeVaersDateRange}">
        <div>
            <label class="text-grey"><i>${Constants.Commons.LAST_EXECUTED_DATE_RANGE} (VAERS): ${exeVaersDateRange}</i>
            </label>
        </div>
    </g:if>
</g:if>
...
<script>
    $(document).ready(function () {
        var showAlertGroups = function () {
            $("#alertGroup").select2({
                allowClear: true,
                // multiple: true,
                placeholder: "Select an alert group",
                width: 'resolve'  // Adjusts width based on the select2 container size
            }).val(null).trigger('change');  // Clear any default selection
        };


            showAlertGroups();
            if(${configurationInstance?.excludedAlertGroups?:false}) {
                $('#alertGroupDiv').show();
                var excludedAlertGroups = "${configurationInstance?.excludedAlertGroups}".split(',');
                $('#alertGroup').val(excludedAlertGroups).trigger('change');
            }
        $('#alertGroupCheckBox').change(function() {
            if ($(this).is(':checked')) {
                $('#alertGroupDiv').show();
            } else {
        ...
        $('#productHierarchySelectDropDown').change(function() {
            var selectedValue = $(this).val();
            $("#showProductSelection").html("");
            if(selectedValue != ''){
                $("#showProductSelection").append("<div style='padding: 5px'> All Products (" + selectedValue + ")</div>");
            }
        });
