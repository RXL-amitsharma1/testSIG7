...
<head>
    ...
    <script>
        $(function(){
            const prodGrp = $("#productGroupSelection").val();
            // Added confirmation pop up if alert is updated and run with different product group as part of bug/PVS-56915
            $("#updateRunButton").on("click", function (event) {
                if (prodGrp !== $("#productGroupSelection").val()) {
                    event.preventDefault();
                    var messageConfirm = "Selected Product is not same as original. This will have impact on PEC History and Activities of the subsequent runs. Please confirm, if you would like to proceed?";
                    bootbox.confirm({
                        message: messageConfirm,
                        title: "Updated Alert Configuration",
                        buttons: {
                            confirm: {
                                label: 'Ok',
                        ...
                        callback: function (result) {
                            if (result) {
                                ...
                                $("#updateRunButton").prop("disabled", false);
                            }
                        }
                    });
                } else {
            ...
            function disableEventSelection() {
                $('#eventDictionarySelection').addClass('disabled');

                $('#eventDictionarySelection .iconSearch a').attr('disabled', true);

                $('#eventDictionarySelection .iconSearch a').click(function (e) {
                    e.preventDefault();
                });
            }
            ...
            function masterTemplateEnabled() {
                $('#groupBySmqLabel').css({
                    ...
                    'pointer-events': 'none',  // Prevent label from interacting
                    'opacity': '0.6'      // Make it appear faded
                });
                $('#isTemplateAlert').prop('checked', false).prop('disabled', true);
                $('#updateRunButton').prop('disabled', true);
                if( $('#alertGroupDiv').is(':checked')){
                    $('#alertGroupDiv').show();
                }
                ...
                if (!$('#nameTextbox').value) {
                    $('#nameTextbox').value = '[Product Hierarchy]-[Product Name]-[Event Hierarchy]';
                }
            }
            ...
            function masterTemplateDisabled() {
                ...
                if ($('#nameTextbox').value == '[Product Hierarchy]-[Product Name]-[Event Hierarchy]') {
                    $('#nameTextbox').value = '';
                }
            }

            if ($('#isMasterTemplateAlert').is(':checked')) {
                $('#aggConfigurationAlertCriteria :input').prop('disabled', true);
            ...
            if (!$("#adhocRun").is(":checked") && !$('#isMasterTemplateAlert').is(':checked')) {
                $('#myScheduler :input').prop('disabled', false);
            ...
            $('#sharedWith').on('select2:unselecting', function(e) {
                if ($('#isMasterTemplateAlert').is(':checked') && e.params.args.data.id === 'AUTO_ASSIGN') {
                    e.preventDefault(); // Prevent removal if the fixed option is selected for removal
                }
            });
            ...
            $('#groupBySmq').on('change', function () {
                if($(this).is(':checked') && !$('#isMasterTemplateAlert').is(':checked')){
                    $('#selectedDatasheet').prop('checked',false)
                    var dataSheetOptions = $(".datasheet-options");
                    dataSheetOptions.hide();
                }
...
<div class="row">
    <div class="col-sm-12">
        ...
        <form id="configurationForm" name="configurationForm" method="POST" action="${createLink(controller: 'aggregateCaseAlert', action: 'run', params: [id: configurationInstance.id])}">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            <g:render template="form" model="[dataSourceMap: dataSourceMap, configurationInstance: configurationInstance, userList: userList, priorityList: priorityList, templateList: templateList, spotfireEnabled:spotfireEnabled,isPVCM: isPVCM, productHierarchy:productHierarchy, eventHierarchy:eventHierarchy, configurationInstanceEventHierarchy:configurationInstanceEventHierarchy,configurationInstancePrdHierarchy:configurationInstancePrdHierarchy]"/>
            ...
            <div class="text-right">
                <g:if test="${configurationInstance?.isEnabled && configurationInstance?.nextRunDate}">
                    <g:actionSubmit accesskey="d" class="btn btn-primary repeat disable"
                                    data-action="${createLink(controller: 'aggregateCaseAlert', action: 'disable', params: [id: configurationInstance.id])}"
                                    type="submit" value="${message(code: "default.button.unschedule.label")}"/>
                </g:if>
                <g:else>
                    <button id="updateRunButton" class="btn btn-primary repeat disable" type="button">${message(code: 'default.button.update.run.label')}</button>
                </g:else>
                <g:actionSubmit accesskey="u" class="btn btn-primary repeat disable"
                                data-action="${createLink(controller: 'aggregateCaseAlert', action: 'update', params: [id: configurationInstance.id])}"
                                type="submit" value="${message(code: 'default.button.update.label')}"/>
        ...
        <div>
            ...
            <g:render template='/templateQuery/templateQuery'
                      model="['templateQueryInstance': null, 'i': '_clone', 'hidden': true]"/>
            %{--</tbody>--}%

            <div class="expression" hidden="hidden"><g:render template="/query/toAddContainerQEV"
                                                              model="[type: 'qev']"/></div>

            <div class="expression" hidden="hidden"><g:render template="/query/customSQLValue"
                                                              model="[type: 'qev']"/></div>
        </div>
    </div>
</div>
...
<script>
    ...
    #eventDictionarySelection.disabled {
        pointer-events: none;
        opacity: 0.5;
        background-color: #dbdbdb;
    }
