...
<head>
    ...
    <g:javascript>
            ...
            var assignToGroupUrl = "${createLink(controller: 'literatureAlert', action: 'changeAssignedToGroup', params:['datasource': selectedDatasource])}";
            ...
            var changeDispositionUrl = "${createLink(controller: 'literatureAlert', action: 'changeDisposition', params: [callingScreen: callingScreen, 'datasource': selectedDatasource])}";
            var revertDispositionUrl = "${createLink(controller: 'literatureAlert', action: 'revertDisposition', params: [callingScreen: callingScreen, 'datasource': selectedDatasource])}";
            var changePriorityUrl = "${createLink(controller: "literatureAlert", action: 'changePriorityOfAlert', params:['datasource': selectedDatasource])}";
            var fetchTagsUrl = "${createLink(controller: 'literatureAlert', action: 'alertTagDetails', params: [isArchived: isArchived, 'datasource': selectedDatasource])}";
            var saveTagUrl = "${createLink(controller: 'literatureAlert', action: 'saveAlertTags', params: [isArchived: isArchived, 'datasource': selectedDatasource])}";
            ...
            var changeAlertLevelDispositionUrl = "${createLink(controller: 'literatureAlert',action: 'changeAlertLevelDisposition', params:['exConfigId': configId, 'datasource': selectedDatasource])}";
            ...
            var fetchSignalsUrl = "${createLink(controller: 'validatedSignal', action: 'fetchSignalNotInAlertObj')}";
            var dispositionDropdownEnabled = "${grailsApplication.config.alert.disposition.useDropdown ? true : false}"
        ...
        <div class="col-md-12 p2">
            ...
            <g:else>
                ...
                <span tabindex="0" class="saveViewPanel grid-menu-tooltip"  accesskey="s" aria-expanded="true"  title="">
                      <a href="${createLink(controller: 'literatureAlert', action: 'view', params: [id: literatureConfigurationId, isExecutedCase: true]) ?: '#'}"
        ...
        <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="details">
                <g:render template="includes/details_tab"
                          model="[viewId: viewId, alertDispositionList: alertDispositionList, name: name, dateRange: dateRange, detailsisArchived: isArchived, configId: configId, litHelpMap: litHelpMap, isAbstractView: isAbstractView]"/>
            </div>
