...
<head>
    ...
    <asset:javascript src="app/pvs/embaseQuery/templateEmbaseQueries.js"/>
    ...
    <g:javascript>
        ...
        $(function () {
            var data = ($('#literatureQueryName').val() && $("#literatureQueryId").val()) ? {
            id: $("#literatureQueryId").val(),
            text: $('#literatureQueryName').val()
            } : {};
            $(".priority-List").find(".select2").select2();
            changeLiteratureDataSource();
            bindSelect2WithUrl($('#literatureQuery'), '/signal/query/queryList', data, true);
            fetchLiteratureQueryDetails();
            copyToClipboardSearchString();
        });
    </g:javascript>
</head>
